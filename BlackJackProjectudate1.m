...
%% Place Bets Function
    % Create a new column in the player table for
    % how much they are betting.
    % Interact with the player's balance to add/subtract
    % when they win/lose
...
for i = 1:numPlayers
    ...
    players(i).hand = table(); % empty hand         % this needs to be filled in later
...
for i = 1:numPlayers 
    playerTotal = sum(players(i).hand.values); % these values need to be calcualted from card values !!!
    dealerTotal = sum(dealer.hand.values); % also these so work on these later lololol=
